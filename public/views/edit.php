...
foreach ($idFilms as $key) {
	echo '<form method="post" action="">
		<div class="form-group">
			...
			<input type="text" class="form-control disabled" name="id_film" value="' . $key['id_film'] . '">
		</div>
		<div class="form-group">
			<label for="titre">Titre :</label>
			<input type="text" class="form-control" name="titre" value="' . $key['titre'] . '">
		</div>
		<div class="form-group">
			<label for="resum">Contenu :</label>
			<textarea name="resum" class="form-control" name="content" cols="30" rows="10">' . $key['resum'] . '</textarea>
		</div>
